...
async function sendEmail(to, subject, html) {
  ...
  try {
    ...
    console.log('sendEmail response:', data, error);
    return { data, error };
  } catch (err) {
        ...
        <div style={{ position: 'absolute', bottom: '100%', left: 0, right: 0, marginBottom: 4, background: 'white', border: '1px solid #e0e0e0', borderRadius: 8, boxShadow: '0 4px 12px rgba(0,0,0,0.1)', overflow: 'hidden', zIndex: 50 }}>
          {filtered.map(p => (
            <div key={p.id} onMouseDown={() => pickMention(p)}
              ...
              <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#1a2744', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, fontWeight: 700, flexShrink: 0 }}>{p.initials}</div>
              <div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'white', borderRadius: 14, padding: 28, width: 400, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <input value={phone} onChange={e => { setPhone(e.target.value); setPhoneError(''); }} placeholder="5095551234 or +15095551234" style={{ ...inp, borderColor: phoneError ? '#e57373' : '#ddd' }} />
      ...
      <div style={{ background: 'white', borderRadius: 14, padding: 28, width: 400, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
          <div onClick={() => fileRef.current?.click()}
            ...
            {avatarPreview ? <img src={avatarPreview} alt="avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 24, fontWeight: 700, color: '#1a2744' }}>{initials || '?'}</span>}
          </div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#333', marginBottom: 3 }}>Profile photo</div>
            <button onClick={() => fileRef.current?.click()} style={{ fontSize: 12, color: '#1a2744', background: 'none', border: '1px solid #ddd', borderRadius: 6, padding: '4px 10px', cursor: 'pointer' }}>
...
function App() {
  ...
  async function addProperty() {
    ...
    const { data, error } = await supabase.from('properties').insert([{
      address: newAddress, status: 'rehabbing',
  ...
  async function saveEdit() {
    ...
    await supabase.from('properties').update({ address: editProp.address, price: editProp.price, status: editProp.status }).eq('id', editProp.id);
  ...
  async function postNote(propertyId) {
    ...
    for (const mp of mentionedProfiles) {
      // Skip if no email
      if (!mp.email) {
        console.warn('Skipping - no email for:', mp.name);
        continue;
      }
      ...
      console.log('Sending email to:', mp.email);
      const result = await sendEmail(mp.email, subject, html);
      console.log('Email result:', result);

      if (mp.phone) {
        await sendSMS(mp.phone, `🏠 ${currentProfile.name} mentioned you in ${prop?.address || 'a property'}: "${newNote.slice(0, 100)}"\nView: ${link}`);
      }
    }
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', background: '#f0ede8', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#1a2744', height: 58, padding: '0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {overdueCount > 0 && (
            <div style={{ background: '#c62828', color: 'white', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>⚠ {overdueCount} overdue</div>
          )}
      ...
      {showSettings && (
        <div style={modalStyle}>
          <div style={{ ...cardStyle, width: 520 }}>
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#aaa', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Users & permissions</div>
              {profiles.map(u => (
                <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: '#fafafa', borderRadius: 8, marginBottom: 6, border: '1px solid #f0f0f0' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#aaa' }}>{u.email} · {u.company} {u.phone ? '· ' + u.phone : ''}</div>
                  </div>
            ...
            <div style={{ marginTop: 20, padding: 12, background: '#f0f7ff', borderRadius: 8, border: '1px solid #d0e8ff' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#1a3a5c', marginBottom: 3 }}>Signed in as {currentProfile?.role === 'admin' ? 'Admin' : 'Member'}</div>
            ...
            return (
              <div key={st} style={{ flex: '0 0 190px' }}>
                ...
                {cols.map(p => {
                  ...
                  return (
                    <div key={p.id} style={{ background: 'white', border: selected?.id === p.id ? '2px solid #1a2744' : hasOverdue ? '1.5px solid #e57373' : '1.5px solid #ebebeb', borderRadius: 10, padding: 12, cursor: 'pointer', marginBottom: 7, position: 'relative' }}>
                      ...
                      {isAdmin && (
                        <button onClick={e => { e.stopPropagation(); setEditProp(p); setShowEdit(true); }}
                          style={{ position: 'absolute', top: 8, right: 8, background: 'none', border: 'none', cursor: 'pointer', color: '#ccc', fontSize: 14, padding: 2 }}>✎</button>
                      )}
        ...
        {selectedProp && (
          <div style={{ width: 280, flexShrink: 0, background: 'white', borderLeft: '1px solid #eee', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 58px)' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '14px 12px 10px' }}>
              ...
              {selectedNotes.map(n => (
                <NoteItem key={n.id} n={n} noteReactions={reactions[n.id] || []} profiles={profiles} currentUser={currentProfile} onToggleReaction={toggleReaction} onEditNote={editNote} onDeleteNote={deleteNote} />
              ))}
            </div>
          ...
          <div style={{ width: 300, flexShrink: 0, background: 'white', borderLeft: '1px solid #eee', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 58px)', overflow: 'hidden' }}>
            <div style={{ padding: 14, borderBottom: '1px solid #f0f0f0' }}>
              ...
              <div style={{ display: 'flex', gap: 5, marginTop: 9 }}>
                {STATUSES.indexOf(selectedProp.status) > 0 && <button onClick={() => moveStatus(selectedProp, -1)} style={{ flex: 1, padding: '6px 0', borderRadius: 7, border: '1px solid #ddd', background: 'white', cursor: 'pointer', fontSize: 11, fontWeight: 500 }}>← Back</button>}
                {STATUSES.indexOf(selectedProp.status) < 4 && <button onClick={() => moveStatus(selectedProp, 1)} style={{ flex: 1, padding: '6px 0', borderRadius: 7, border: 'none', background: '#1a2744', color: 'white', cursor: 'pointer', fontSize: 11, fontWeight: 500 }}>Next Stage →</button>}
              </div>
            </div>
              ...
              {tab === 0 && (
                <div>
                  ...
                  {selectedChecks.map(check => {
                    ...
                    return (
                      <div key={check.id} style={{ padding: '7px 9px', background: overdue ? '#fff5f5' : '#fafafa', borderRadius: 7, marginBottom: 5, border: overdue ? '1px solid #ffcdd2' : soon ? '1px solid #ffe0b2' : '1px solid #f0f0f0' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                          <input type="checkbox" checked={check.is_done} onChange={() => toggleCheck(check)} style={{ width: 14, height: 14, accentColor: '#1a2744', cursor: 'pointer', flexShrink: 0 }} />
                          <span style={{ flex: 1, fontSize: 12, color: check.is_done ? '#bbb' : overdue ? '#c62828' : '#333', textDecoration: check.is_done ? 'line-through' : 'none', lineHeight: 1.3 }}>{check.label}</span>
                        </div>
                  ...
                  {isAdmin && (
                    <div style={{ marginTop: 10 }}>
                      ...
                      <div style={{ display: 'flex', gap: 5 }}>
                        <input value={newCheckLabel} onChange={e => setNewCheckLabel(e.target.value)} placeholder="New item..." style={{ flex: 1, padding: '6px 8px', borderRadius: 7, border: '1px solid #e0e0e0', fontSize: 12, outline: 'none' }} />
                        <button onClick={() => { addCheck(selectedProp.id, selectedProp.status, newCheckLabel); setNewCheckLabel(''); }} style={{ background: '#1a2744', color: 'white', border: 'none', borderRadius: 7, padding: '6px 9px', fontSize: 11, cursor: 'pointer' }}>Add</button>
                      </div>
                    </div>
                  )}
                </div>
